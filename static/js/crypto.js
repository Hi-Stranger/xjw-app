...
var KEY = "abcdefghjklksdfjsajdjsdferjijssa";
...
function testEnCrypt(){
    var word = {a:123,b:234};
    var strKey = "abcdefghjklksdfjsajdjsdferjijssa";//秘钥必须为：8/16/32位
    // var enResult = doEnCrypt(word,strKey);
    // console.log("word: "+word);
    // console.log("strKey: "+strKey);
    // console.log("encrypt_value1: "+enResult);
    // console.log("encrypt_value2: "+enResult.ciphertext.toString());
    var deResult = doDecrypt("9e43c87d62fb965a65e5ee3d8ae4a8fa82b6b5fad03954a5cfa5c1f5ef87c9b3", strKey);
    deResult = JSON.parse(deResult);
    console.log("ssd...",deResult,deResult.a,deResult.b);
}
...
//AES加密
function doEnCrypt(strSrc, strKey){
    var encrypted = "";
    if(CryptoJS){
        ...
        if (typeof(strSrc) == 'string') {
            var src = CryptoJS.enc.Utf8.parse(strSrc);
            encrypted = CryptoJS.AES.encrypt(src, key, {
                iv: iv,
                mode: CryptoJS.mode.CBC,
                padding: CryptoJS.pad.Pkcs7
            });
        } else if (typeof(strSrc) == 'object') {//对象格式的转成json字符串
            var data = JSON.stringify(strSrc);
...
//AES解密
function doDecrypt(strSrc, strKey){
    var dncrypted = "";
    if(CryptoJS){
        // strSrc = strSrc.toString();
        var key = CryptoJS.enc.Utf8.parse(strKey);
        var iv = CryptoJS.enc.Utf8.parse(strKey);
        var encryptedHexStr = CryptoJS.enc.Hex.parse(strSrc);
        var src = CryptoJS.enc.Base64.stringify(encryptedHexStr);
        dncrypted = CryptoJS.AES.decrypt(src, key, {
            iv: iv,
            mode: CryptoJS.mode.CBC,
            padding: CryptoJS.pad.Pkcs7
        });
